...
from audit_workflow.models import Notification
...
def audit_summary(request, audit_id):
    ...
    # ✅ Always initialize form objects (avoids UnboundLocalError)
    ...
    # 🔁 Convicted persons per item
    convicted_persons_by_item = {
        item.id: Person.objects.filter(audit_id=audit, items=item)
        for item in current_page_items
    }

    # ✅ Status mapping (to mark ✅❌)
    item_statuses = {
        status.item.id: status.is_completed
        for status in AuditItemStatus.objects.filter(audit=audit, item__in=current_page_items)
    }

    # ✅ If user clicked an item from the left panel (AJAX GET)
    if request.method == "GET" and 'item_id' in request.GET:
        ...
        person_formset = PersonFormSet(queryset=Person.objects.filter(audit_id=audit, items=selected_item),
                                       prefix='person_set')
    ...
    # ✅ Completed objections only for this page
    completed_objections = AuditObjection.objects.filter(
        submission=audit,
        items__in=current_page_items
...
def dashboard(request):
    ...
    context = {
        ...
        'user_role': user.role,
        'unread_notifications': request.user.notifications.filter(is_read=False),  # ✅ Add this line
    }
...
def audit_detail(request, audit_id):
    ...
    objections = audit.auditobjection_set.select_related('decision', 'items').all().order_by('items__itemNo')
    paginator = Paginator(objections, 10)
    page_number = request.GET.get('page')
    page_obj = paginator.get_page(page_number)

    context = {
        'audit': audit,
        'audit_objections': objections,  # still needed for modal fallback
        'page_obj': page_obj,
    }
...
def user_login(request):
    if request.user.is_authenticated:
        return redirect('main:home')  # if already logged in
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if user is not None:
                login(request, user)
                return redirect('main:home')  # redirect after login
            else:
...
def comment_detail(request, audit_id, item_id):
    ...
    comments = Comment.objects.filter(item=item, submission=submission).order_by('comment_date')

    user = request.user
    user_role = get_effective_user_role(user, submission)
    is_regular = user_role == 'regular'
    is_manager = user_role == 'manager'
    is_rao = user_role == 'rao'
    is_admin = user.is_superuser

    is_closed = objection.decision.decision == 'close' if objection and hasattr(objection, 'decision') else False

    # Check comment/reply conditions
    has_user_commented = comments.filter(commented_by=user).exists()
    top_level_comment = comments.filter(parent=None).first()

    can_comment = user_role == 'regular' and not is_closed and not has_user_commented
    can_reply = False
    ...
    if top_level_comment and not is_closed:
        ...
        if latest_comment.commented_by != user:
            if (get_effective_user_role(latest_comment.commented_by,
                                        submission) == 'manager' and user_role == 'regular') or \
                    (get_effective_user_role(latest_comment.commented_by,
                                             submission) == 'regular' and user_role == 'manager'):
                can_reply = True
    ...
    for comment in comments.filter(parent=None):
        ...
        if not is_closed and latest.commented_by != user:
            ...
            if (latest_role == 'manager' and user_role == 'regular') or (
    ...
    if request.method == "POST":
        if 'comment_form' in request.POST and can_comment:
            ...
            if comment_form.is_valid():
                ...
                predefined_text = request.POST.get('predefined_reply', '')
                if predefined_text:
                    new_comment.comment = predefined_text
                new_comment.save()
                submission.is_authorized = False  # Invalidate authorization on new comment
                submission.save()
                return redirect('audit_workflow:comment_detail', audit_id=audit_id, item_id=item.id)
        else:
            comment_id = request.POST.get("reply_submit")
            if comment_id and comment_id.isdigit() and int(comment_id) in can_reply_dict and can_reply_dict[
                int(comment_id)]:
                ...
                if form.is_valid():
                    ...
                    predefined_text = request.POST.get(f'{prefix}-predefined_reply', '')
                    typed_comment = form.cleaned_data.get('comment', '')
                    comment_text = predefined_text or typed_comment
                    if comment_text:
...
def download_jaripotro_docx(request, audit_id):
    ...
    user = request.user
    if submission.authorized_by and submission.authorized_by.branch:
        center_para.add_run(f"{submission.authorized_by.branch.bn_name}")
    else:
        center_para.add_run("অননুমোদিত")
    ...
    data = [
        {
            ...
            'message': n.message,
            'audit_id': n.audit.id,
            'url': f"/audit_workflow/audit_detail/{n.audit.id}/",
